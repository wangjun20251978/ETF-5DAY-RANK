...
DEMO = [
    ...
    ("sh588170","科创半导体",1.019,4.09,12.97,8.06,5.7,94.3,1.12,56.1,48.36,True),
    ...
    ("sh589720","科创新药",0.879,2.09,-1.46,-4.06,30.1,69.9,0.86,43.0,32.25,False),
]


def fetch_kline(symbol, datalen=160, tries=3):
    """从新浪财经抓取日K线, 返回按时间升序的 [{day,close,volume}]
    带重试: 新浪接口偶发 HTTP 456 限流, 重试可显著降低失败率"""
...
def backtest(ranks, matrix, code, signal_n=None):
    """历史回测: 信号触发后持有[1,3,5,10,20]天收益, 取均值最高为最优持股天数
    signal_n: 信号口径取综合前N名, 默认 BT_SIGNAL_N(统计口径, 样本更足)"""
    n_top = signal_n if signal_n else BT_SIGNAL_N
    ...
    for idx in range(L):
        if idx + maxk >= L:
            break
        day = ranks[idx]
        top = set(x[0] for x in day[:n_top])
        fc = facs[idx]
...
def render(records, now_str, live, market=None):
    ...
    # ---- 策略口径排名: 按综合得分降序(用于选持仓/信号天数, 保持策略不变) ----
    by_total = sorted(records, key=lambda r: r["total"], reverse=True)
    ...
    if market["state"] == "强势":
        ...
        hold_n = MAX_HOLD
    passers = [r for r in by_total if r["pass"]]
...
.low{color:var(--y);font-size:10px;}
    ...
    if live:
        for code, name in ETFS:
            ...
            if rows:
                ...
                # 单只实时抓取失败 -> 用离线快照兜底, 避免整行消失(5日/5日均显示—)
                try:
